...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const offer = await Offer.findOne({ _id: id, type: 'flight' }).populate('createdBy updatedBy', 'firstName lastName email').lean()
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const offer = await Offer.findOneAndUpdate({ _id: id, type: 'flight' }, { ...body, updatedBy: session.user.id }, { new: true, runValidators: true }).populate('createdBy updatedBy', 'firstName lastName email').lean()
    if (!offer) return NextResponse.json({ error: 'Oferta no encontrada' }, { status: 404 })
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (isPermanent) {
      ...
      const offer = await Offer.findOneAndDelete({ _id: id, type: 'flight' }).lean()
      ...
      const offer = await Offer.findOneAndUpdate({ _id: id, type: 'flight' }, { status: 'archived', updatedBy: session.user.id }, { new: true }).lean()
      if (!offer) return NextResponse.json({ error: 'No encontrada' }, { status: 404 })
