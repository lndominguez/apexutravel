...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters: any = { type: 'transport' }
    ...
    return NextResponse.json({ error: 'Error al obtener ofertas de transporte', details: error.message }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newOffer = await new Offer({ ...body, type: 'transport', createdBy: session.user.id }).save()
    const populatedOffer = await Offer.findById(newOffer._id).populate('createdBy', 'firstName lastName email').lean()
    return NextResponse.json({ message: 'Oferta de transporte creada exitosamente', offer: populatedOffer }, { status: 201 })
  } catch (error: any) {
